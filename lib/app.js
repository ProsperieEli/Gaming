...
app.get('/games_owned', async(req, res) => {
  try {
    const data = await client.query('SELECT * from games_owned');
...
app.get('/games_owned/:id', async(req, res) => {
  try {
    const data = await client.query('SELECT * from games_owned WHERE id = $1', [req.params.id]);
...
app.post('/games_owned', async(req, res) => {
  try{
    ...
    VALUES ($1, $2) RETURNING *`, [req.body.name, 1]);
